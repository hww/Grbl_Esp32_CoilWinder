  ...
  (letrec ([wradius (/ wdiam 2)]
    ...
    (let ([direction 1] 
      ...
      (for ([i (in-range 0 turns)])
        (letrec ([i1 (+ i 1)]
                 [is_last_turn (= 0 (modulo i1 turns_per_row))]
                 ...
                 [turn_width (if is_last_turn (* 0.5 wdiam) wdiam)]
                 [posx_old posx])
          ;;(printf (format "Crossover Start: X~6,6F  Ends: Y~6,6F" cross_starts cross_ends))
          ;; Increment or decrement X pposition
          (if (= direction 1)
              (set! posx (+ posx turn_width))
              (set! posx (- posx turn_width)))
...
(define (build_gcode_mm length diam turns rpm wdiam)
